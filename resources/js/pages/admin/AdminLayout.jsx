...
export default function AdminLayout() {
  ...
  // Fetch profile once at layout level so both sidebar + topbar share it
  React.useEffect(() => {
    ...
    <div className="min-h-screen bg-white text-neutral-900">
      <div className="flex">
        ...
        <div className="flex-1">
          <AdminTopbar profile={profile} onProfileUpdate={setProfile} />
...
function AdminSidebar({ profile }) {
  ...
  // Avatar: use uploaded photo, else DiceBear seeded with name
  const sidebarAvatar = profile?.profile_photo_url
  ...
  return (
    <aside className="w-[260px] border-r border-neutral-200 bg-white">
      ...
      <div className="px-5 py-5">
        <p className="mb-2 text-[10px] tracking-widest text-neutral-500">MAIN NAVIGATION</p>
        ...
        <p className="mb-2 text-[10px] tracking-widest text-neutral-500">SECOND NAVIGATION</p>
        ...
        <p className="mb-2 text-[10px] tracking-widest text-neutral-500">SYSTEM / ACCOUNT</p>
        <nav className="space-y-1">
          {systemNav.map((item) => (
            <SidebarLink key={item.to} to={item.to} active={isActive(item.to)}>
              <GridIcon />
...
function EditProfileModal({ profile, onClose, onSaved }) {
  ...
  const [preview, setPreview] = React.useState(profile?.profile_photo_url ?? null);

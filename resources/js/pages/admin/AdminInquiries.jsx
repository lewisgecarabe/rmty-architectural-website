...
export default function AdminInquiries() {
    ...
    useEffect(() => {
        ...
        // Fetch which platforms THIS logged-in user has connected
        Promise.all([
            apiFetch("/admin/google/status"),
            apiFetch("/admin/facebook/status"),
        ])
            .then(([googleData, fbData]) => {
                setPlatformStatus({
                    gmail:     !!googleData.connected,
                    facebook:  !!fbData.facebook?.connected,
                    instagram: !!fbData.instagram?.connected,
                });
            })
            .catch(() => {});
        ...
        <div className="flex flex-col [font-family:var(--font-neue)] relative pb-10">
                    ...
                    <div className="flex flex-col sm:flex-row items-center w-full sm:w-auto mt-3 sm:mt-0 sm:ml-3">
                        <motion.div layout className="w-full sm:w-44 shrink-0">
                            <AnimatedSelect
                                ...
                                options={Object.entries(PLATFORM_LABELS).map(([k, v]) => ({ id: k, name: v }))}
                                className="py-2.5"
                                onChange={(id) => setFilter("platform", id)}
                            />
                        </motion.div>

                        <motion.div layout className="w-full sm:w-40 shrink-0 mt-3 sm:mt-0 sm:ml-3">
                            <AnimatedSelect
                        ...
                        <motion.div layout className="flex flex-col sm:flex-row items-center w-full sm:w-auto mt-3 sm:mt-0 sm:ml-3">
                            <AnimatePresence>
                                {(filters.search || filters.platform || filters.status) && (
                                    ...
                                    >
                                        ...
                                        >
                                            <button
                                                onClick={() => {
                                                    setFilters({ search: "", platform: "", status: "" });
                        ...
                        {!loading && inquiries.length === 0 ? (
                            ...
                            <table className="w-full text-left border-collapse whitespace-nowrap">
                                <thead className="sticky top-0 bg-neutral-50 border-b border-neutral-100 z-10">
                                    <tr>
                                        <th className="px-5 py-4 text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase border-b border-neutral-200">Sender</th>
                                        <th className="px-5 py-4 text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase border-b border-neutral-200">Platform</th>
                                        <th className="px-5 py-4 text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase border-b border-neutral-200 hidden md:table-cell w-1/3">Message</th>
                                        <th className="px-5 py-4 text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase border-b border-neutral-200">Status</th>
                                        <th className="px-5 py-4 text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase border-b border-neutral-200 text-right">Date</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-neutral-100">
                                    {inquiries.map((item) => (
                                        <tr
                                            ...
                                            className={`group cursor-pointer transition-colors hover:bg-neutral-50 h-[73px] ${selected?.id === item.id ? "bg-neutral-50" : ""}`}
                                        >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-neutral-900 truncate max-w-[150px]">
                                                            {item.name || item.first_name || "Unknown"}
                                                        </p>
                                                        {item.email && (
                                            ...
                                            <td className="px-5 py-4 align-middle">
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${PLATFORM_COLORS[item.platform] ?? "bg-neutral-50 text-neutral-600 border-neutral-200"}`}>
                                            ...
                                            <td className="px-5 py-4 align-middle">
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${STATUS_COLORS[item.status] ?? "bg-neutral-50 text-neutral-600 border-neutral-200"}`}>
                                                    {item.status}
                                                </span>
                                            </td>
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-t border-neutral-100 bg-neutral-50/50 mt-auto rounded-b-2xl">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    onClick={() => { setPage((p) => p - 1); load(page - 1, filters); }}
                    ...
                    <>
                        ...
                        >
                            <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-100 bg-neutral-50/50 shrink-0">
                                <h3 className="text-sm font-bold text-neutral-900 uppercase tracking-widest">Inquiry Details</h3>
                                <button onClick={() => setSelected(null)} className="text-neutral-400 hover:text-black transition-colors outline-none cursor-pointer">
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
                                <div>
                                    <p className="text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase mb-1">Sender</p>
                                    <p className="text-2xl font-black text-neutral-900 leading-tight">
                                        {selected.name || selected.first_name || "Unknown"}
                                    </p>
                                    {selected.email && <p className="text-sm font-medium text-neutral-600 mt-1">{selected.email}</p>}
                                    {selected.phone && <p className="text-sm font-medium text-neutral-600 mt-1">{selected.phone}</p>}
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <p className="text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase mb-2">Platform</p>
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${PLATFORM_COLORS[selected.platform] ?? "bg-neutral-50 text-neutral-600 border-neutral-200"}`}>
                                    ...
                                    <div>
                                        ...
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${STATUS_COLORS[selected.status] ?? "bg-neutral-50 text-neutral-600 border-neutral-200"}`}>
                                            {selected.status}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <p className="text-[10px] font-bold tracking-[0.15em] text-neutral-400 uppercase mb-2">Received At</p>
                            ...
                            <div className="p-6 border-t border-neutral-100 bg-neutral-50/50 space-y-3 shrink-0">
                                {/* ✅ Reply button gated by platform connection status */}
                                {canReply(selected) ? (
                                    <button
                                        onClick={() => { setReplyId(selected.id); setReplyMsg(""); }}
                                    ...
                                    <div className="w-full flex items-center justify-center gap-2 rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3.5">
                                        <LockIcon className="w-4 h-4 text-neutral-400" />
                                        <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider">
                                            Connect {PLATFORM_LABELS[selected.platform] ?? selected.platform} to Reply
                                        </span>
                                    </div>
                                )}
            ...
            <AnimatePresence>
                {deleteId && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                    >
                        <div className="absolute inset-0 bg-black/20 cursor-pointer" onClick={() => setDeleteId(null)} />
                        <motion.div
                            initial={{ opacity: 0, scale: 0.95, y: 10 }}
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.95, y: 10 }}
                        ...
                        >
                            ...
                            <h3 className="text-xl font-black text-neutral-900 mb-2">Delete Inquiry?</h3>
            ...
            <AnimatePresence>
                {toast && (
                    ...
                    >
                        <div className={`flex items-center gap-3 px-6 py-4 rounded-2xl border ${toast.type === "success" ? "bg-black text-white border-black" : "bg-red-600 text-white border-red-700"}`}>
                            {toast.type === "success" ? <CheckIcon className="w-4 h-4 text-emerald-400" /> : <CloseIcon className="w-4 h-4 text-white" />}
                            <p className="text-[11px] font-bold tracking-widest uppercase mt-0.5">{toast.msg}</p>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>

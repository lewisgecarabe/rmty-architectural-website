  ...
  return (
    <main className={`w-full ${bgColor} min-h-screen pt-32 pb-32 [font-family:var(--font-neue)] text-black overflow-x-hidden`}>
      ...
      <AnimatePresence mode="wait">
        {/* CRITICAL: The 'key={id}' here ensures the animation triggers on every click */}
        <motion.div
          key={id} 
        ...
        >
          ...
          <div className="max-w-screen-2xl mx-auto px-6">
            <div className="w-full flex justify-end mb-8">
              <Link to="/projects" className="text-sm font-semibold underline underline-offset-4 hover:opacity-60 transition-opacity">
                Back to Projects
              </Link>
            </div>
          ...
          <div className="w-full mb-32 overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[400px] md:h-[400px] lg:h-[500px]">
              ...
              {/* PREVIOUS (Hidden Mobile) */}
              <Link to={`/projects/${prevProject?.id}`} className="hidden md:block relative w-full h-full group bg-[#d9d9d9]">
                {prevProject?.image && <img src={`/storage/${prevProject.image}`} className="absolute inset-0 w-full h-full object-cover" />}
                ...
                <div className="absolute inset-0 flex items-center justify-center z-20 transition-transform group-hover:-translate-x-2">
                  <div className="border-2 border-black w-12 h-12 flex items-center justify-center bg-white/20 backdrop-blur-sm">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" /></svg>
                  </div>
                </div>
              </Link>
              ...
              <div className="relative w-full h-full bg-[#d9d9d9] overflow-hidden">
                <img src={`/storage/${project.image}`} className="w-full h-full object-cover" />
                
                {/* Mobile Arrows Layer */}
                <div className="absolute inset-0 flex justify-between items-center px-4 md:hidden z-30 pointer-events-none">
                  <Link to={`/projects/${prevProject?.id}`} className="pointer-events-auto border-2 border-black w-12 h-12 flex items-center justify-center bg-white/40 backdrop-blur-md">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" /></svg>
                  </Link>
                  <Link to={`/projects/${nextProject?.id}`} className="pointer-events-auto border-2 border-black w-12 h-12 flex items-center justify-center bg-white/40 backdrop-blur-md">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
                  </Link>
                </div>
              </div>

              {/* NEXT (Hidden Mobile) */}
              <Link to={`/projects/${nextProject?.id}`} className="hidden md:block relative w-full h-full group bg-[#d9d9d9]">
                {nextProject?.image && <img src={`/storage/${nextProject.image}`} className="absolute inset-0 w-full h-full object-cover" />}
                <div className="absolute inset-0 bg-gradient-to-l from-[#f5f5f5] via-transparent to-transparent opacity-90 z-10" />
                <div className="absolute inset-0 flex items-center justify-center z-20 transition-transform group-hover:translate-x-2">
                  <div className="border-2 border-black w-12 h-12 flex items-center justify-center bg-white/20 backdrop-blur-sm">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
                  </div>
                </div>
              </Link>
            </div>
          ...
          <div className="max-w-screen-2xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {moreProjects.map((p, idx) => (
                <ProjectCard key={idx} title={p.title} category={p.location} slug={p.slug || p.id} image={p.image} colSpan="col-span-1" aspectRatio="aspect-square" />
              ))}
            </div>
          </div>
        </motion.div>
